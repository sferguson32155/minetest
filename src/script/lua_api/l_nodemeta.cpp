...
void NodeMetaRef::reportMetadataChange()
{
	...
	MapBlock *block = m_env->getMap().getBloc kNoCreateNoEx(blockpos);
	if (block) {
		block->raiseModified(
...
NodeMetaRef::NodeMetaRef(v3s16 p, ServerEnvironment *env)
    : m_p(p), m_env(env), m_is_local(false)
{
}
...
const luaL_Reg NodeMetaRef::methodsServer[] = {luamethod(MetaDataRef, get_string),
...
const luaL_Reg NodeMetaRef::methodsClient[] = {luamethod(MetaDataRef, get_string),
